...
export default {
	...
	methods: {
		...
		saveSubreddits() {
			chrome.storage.sync.set({blockedSubreddits: this.subreddits}, () => {});
		},
		addSubreddit() {
...
<template>
	...
	<div class="card card-border bg-base-100">
					...
					<div class="join w-full">
						<div class="w-full">
							<label class="input join-item w-full">
								/r/
								<input type="text" placeholder="Enter subreddit" class="w-full" v-model="newSubreddit"/>
							</label>
						</div>
						<input type="submit" class="btn btn-success join-item" value="Add"></input>
					</div>
				</form>
			</div>
			<template v-if="subreddits.length">
		...
		</div><div class="card-body" v-if="navigation == 'about'">
			<h2 class="card-title">
				About SubSieve
			</h2>
			<p>
				I originally turned to Reddit for its fun, quirky, and engaging content. Over time, however, I found that the constant influx of negativity, relentless political debates, doom, gloom, and apocalyptic news began to take a toll on my mental health. Much like watching what you eat on a diet, I decided it was time to monitor what I consume online.
            </p>
            <p>
                This extension isn’t about sticking my head in the sand or ignoring the world’s challenges—it’s about choosing when to engage with heavy news and when to enjoy lighthearted entertainment. By filtering out unwanted content on /r/all, I can curate my feed, ensuring that I stay informed on my own terms without overwhelming my wellbeing.
            </p>
            <p>
                I hope this tool empowers you to take control of your digital diet, so you can enjoy Reddit for the fun and inspiration it offers while still having the space to engage with the serious issues when you’re ready.
			</p>
            <p>
